...
sudo git clone --depth=1 $GIT_BASE/openstack-infra/config.git \
    /root/config
#HACK: try out our own puppet scripts
sudo cp /opt/nodepool-scripts/*.pp /root/config/modules/openstack_project/manifests
#HACK

sudo /bin/bash -x /root/config/install_modules.sh
